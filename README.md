...
1. 데이터 불러오기
아파트 실거래, 서울시 경계선, 불러오기
load()함수를 사용하여 필요한, 데이터를 불러온다.

2. 마커 클러스터링 설정
load()로 마커 클러스터링 실행 스크립트를 등록 

3. 반응형 지도 만들기
add.Tiles()함수로 기본 맵을 오픈스트리트맵으로 설정.

addRasterImage()함수로 2021년 최고가 지역과 급등 지역을 나타내는 래스터 이미지를 불러옴.

addLayersControl()함수로 변경 스위치를 설정합니다.

addPloygons()함수로 서울시 외고가 경계선을 불러오고 addCircleMarker()함스로 마커 클러스터링 불러옴.

10-02 지도 애플리케이션 만들기

1. 그리드 필터링하기

1km 단위로 구분한 그리드파일 불러옴.
그리드 데이터는 sf형이므로 필요한 부분만 잘라내는 as로 sp로 바꾼 다음 as로 변환 필요.
